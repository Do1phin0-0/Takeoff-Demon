...
Do not skip image generation when an image-generation tool is available in this environment (e.g. the Higgs `generate_image` / `generate_image_batch` tools).
...
- In this environment, bias strongly toward larger, more analyzable section images.
...
Where a more compact multi-section composition is genuinely appropriate (e.g. a quick moodboard rather than a build reference), this skill may allow it — but section clarity and extraction accuracy are the default priority.
...
When this skill is used in an environment that supports image generation plus implementation, default to an image-first workflow for website design tasks.
...
17. Has the process avoided compressing too many sections into one tiny image?
