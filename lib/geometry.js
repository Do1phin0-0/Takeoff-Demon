...
// This is an absolute tolerance sized for cross-products of canvas-pixel
// coordinates (which run into the thousands), not Number.EPSILON — that
// constant is scaled for values near 1 and is tighter than the rounding
// noise a genuinely collinear point produces at this coordinate scale, so
// it would fail to catch real collinear points here.
...
// Sum of segment lengths along an open polyline (not a closed loop — no
// wraparound edge from the last point back to the first).
...
function computeLinearFootage(polyline, scale) {
  const feetPerPixel = feetPerPixelFromScale(scale);
  const lengthPixels = polylineLengthPixels(polyline);
  const lengthFt = lengthPixels * feetPerPixel;
  return { lengthFt, feetPerPixel, lengthPixels };
}
