...
Maintenance note: Section 8 ("Repo Reality") is a snapshot, not a doctrine. Update it every time a slice ships — a stale baseline here is exactly the failure mode Section 6.1 exists to prevent. Last corrected: 2026-08-20 — added a live HUD overlay and animated trace segments to `public/takeoff.html`, then extracted the segment-distance accumulation into a tested pure module (`public/lib/trace.js`); test count now 61 (5 new unit tests for the extracted module; the HUD itself remains presentation-only over the existing manual flow, no new detection logic — verified with the full suite plus a headless-browser pass through calibrate → trace → close before and after the extraction, same numbers both times).
...
- Live HUD overlay on `takeoff.html` — a status panel (`#hud`) over the canvas showing current step, running perimeter length, and running area, updated on every click during calibration and tracing. Length accumulation runs through `public/lib/trace.js` (`distance`/`processPath`, a native ES module with its own unit tests in `test/trace.test.js`, imported by the browser and by `node --test` alike); area readout still uses the shoelace math local to `takeoff.html`, kept deliberately separate from `lib/geometry.js`'s server-side copy — the client preview and the server's independent verification are not supposed to share code. Each new calibration or trace segment animates drawing in (~180ms) instead of appearing instantly. This is presentation on top of the existing 100%-manual calibrate/trace flow — no boundary detection, no OCR, no auto-scale-read; the user still places every point by hand. Verified with a headless-browser pass through the full calibrate → trace → close-shape sequence, confirming the HUD's live area readout matches the server-verified area exactly
