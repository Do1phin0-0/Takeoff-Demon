...
Maintenance note: Section 8 ("Repo Reality") is a snapshot, not a doctrine. Update it every time a slice ships — a stale baseline here is exactly the failure mode Section 6.1 exists to prevent. Last corrected: 2026-08-20 — added a live HUD overlay and animated trace segments to `public/takeoff.html`; test count unchanged at 56 (presentation-only change to the existing manual flow, no new detection logic, verified with the full suite plus a headless-browser pass through calibrate → trace → close).
